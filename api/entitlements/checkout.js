...
// api-src/entitlements-checkout.ts
var entitlements_checkout_exports = {};
__export(entitlements_checkout_exports, {
  default: () => handler
});
module.exports = __toCommonJS(entitlements_checkout_exports);
...
// node_modules/@clerk/backend/dist/chunk-UOT6YP2O.mjs
...
var LEMONSQUEEZY_API_BASE = "https://api.lemonsqueezy.com/v1";
var createLemonCheckout = async (variantId, customData) => {
...
async function handler(req, res) {
  try {
    ...
    if (kind === "download") {
      ...
      const variantId2 = process.env.LEMONSQUEEZY_VARIANT_ID_DOWNLOAD;
      if (!variantId2) {
        res.status(500).json({ error: "LEMONSQUEEZY_VARIANT_ID_DOWNLOAD is not set on the server" });
        return;
      }
      const checkoutUrl2 = await createLemonCheckout(variantId2, {
        tenantId: tenant.id,
        userId: user.userId,
        signatureId
      });
      res.status(200).json({ checkoutUrl: checkoutUrl2 });
      return;
    }
